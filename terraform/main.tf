...
# List private endpoint connections on APIM after AFD origin is created.
# The connection name is generated by Azure at runtime — we read it dynamically.
data "azapi_resource_list" "apim_pe_connections" {
  type                   = "Microsoft.ApiManagement/service/privateEndpointConnections@2022-08-01"
...
locals {
  pending_pe_connections = {
    for conn in try(jsondecode(data.azapi_resource_list.apim_pe_connections.output).value, []) :
    conn.name => conn
    if try(conn.properties.privateLinkServiceConnectionState.status, "") == "Pending"
  }
}

# Approve each pending AFD → APIM private link connection via azapi (no az CLI needed).
resource "azapi_resource_action" "approve_apim_pe" {
  for_each = local.pending_pe_connections

  type        = "Microsoft.ApiManagement/service/privateEndpointConnections@2022-08-01"
  resource_id = "${azurerm_api_management.lab.id}/privateEndpointConnections/${each.key}"
  method      = "PUT"
