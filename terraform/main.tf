...
###############################################################################
# API Management — internal VNet mode, fronted by Azure Front Door Premium
...
resource "azurerm_api_management" "lab" {
  ...
  # Internal: gateway only reachable from within VNet
  # Front Door Premium connects via Private Link
...
###############################################################################
# Azure Front Door Premium — global entry point → APIM via Private Link
###############################################################################
...
resource "azurerm_cdn_frontdoor_origin" "apim" {
  ...
  private_link {
    request_message        = "AFD private link to APIM gateway"
    location               = azurerm_resource_group.lab.location
    private_link_target_id = azurerm_api_management.lab.id
    target_type            = "Gateway"
  }
}
...
# Auto-approve the AFD → APIM private link connection.
# The connection name is generated by Azure at runtime so we poll for it.
resource "null_resource" "approve_afd_apim_private_link" {
  depends_on = [azurerm_cdn_frontdoor_origin.apim]

  provisioner "local-exec" {
    command = <<-EOT
      echo "Waiting for AFD private link connection to appear on APIM..."
      APIM_ID="${azurerm_api_management.lab.id}"
      for i in $(seq 1 24); do
        CONN=$(az rest --method get \
          --url "https://management.azure.com$${APIM_ID}/privateEndpointConnections?api-version=2022-08-01" \
          --query "value[?properties.privateLinkServiceConnectionState.status=='Pending'].name | [0]" \
          --output tsv 2>/dev/null || true)
        if [ -n "$CONN" ] && [ "$CONN" != "None" ]; then
          echo "Approving private link connection: $CONN"
          az rest --method put \
            --url "https://management.azure.com$${APIM_ID}/privateEndpointConnections/$${CONN}?api-version=2022-08-01" \
            --body "{\"properties\":{\"privateLinkServiceConnectionState\":{\"status\":\"Approved\",\"description\":\"Auto-approved by Terraform\"}}}"
          echo "Private link connection approved."
          exit 0
        fi
        echo "  attempt $i/24 — not yet available, retrying in 15s..."
        sleep 15
      done
      echo "ERROR: private link connection did not appear after 6 minutes" >&2
      exit 1
    EOT
  }
}
